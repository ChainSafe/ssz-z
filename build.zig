...
pub fn build(b: *std.Build) void {
    ...
    const lib_bun_ffi = b.addLibrary(.{
        .name = "bun_ffi",
        .root_module = module_bun_ffi,
        .linkage = .dynamic,
    });

    const install_lib_bun_ffi = b.addInstallArtifact(lib_bun_ffi, .{});
    const tls_install_lib_bun_ffi = b.step("build-lib:bun_ffi", "Install the bun_ffi library");
    tls_install_lib_bun_ffi.dependOn(&install_lib_bun_ffi.step);
    b.getInstallStep().dependOn(&install_lib_bun_ffi.step);
